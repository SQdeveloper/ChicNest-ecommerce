...
const Checkbox = ({onChange, name, text, htmlFor, defaultChecked }: {onChange?: ChangeEventHandler<HTMLInputElement>, defaultChecked?: boolean, name?: string, text: string, htmlFor: string}) => {
    return (
        <div className="w-full">
            ...
            <label className="max-xl:text-xl text-[16px] text-rat-gray border cursor-pointer border-white w-full block px-2 py-2 rounded-md hover:bg-extra-light-gray peer-checked:bg-extra-light-gray" htmlFor={htmlFor}>{text}</label>            
        </div>
    );
};
